...
2. **每条新闻必须是当日（Asia/Shanghai 时区的 ${DATE} 当天，最迟前一天发生）的真实事件**。**绝对禁止**把几天前甚至几周前的旧闻塞进来撑数。如果今日 AI 圈确实清淡，宁可少写也不要硬凑——某个区段写 1-2 条甚至空着都可以（`## 区段` 标题保留，下方写一行 `_今日暂无重要进展_`）。
3. **不强制每个 section 的条数**——根据当日真实新闻量动态决定：
   - 行业大事日（发布会、融资、模型发布）：可写 5-8 条 hero、5-8 条 mini、6-10 条快讯。
   - 平淡日：2-3 条 hero、2-3 条 mini、3-5 条快讯也完全合理。
   - 重要程度排序：把信息密度最高、影响最广的事件放进 hero；中等放 mini；纯短讯/数字播报放 quick。
4. **重点新闻（Agent 重点 hero 条）摘要写 3-4 句**，含至少 1 个**关键数字**。
5. **行业动态和快讯每条必须有 [来源](URL) 标注**，URL 必须是真实可访问的网页。
6. 不要用"震惊"、"颠覆"、"史上最强"这类标题党词汇；用客观、动作动词（推出、宣布、融资、开源）。
7. 标题里若涉及公司/产品，**首次出现写全称**（如"Anthropic（Claude 母公司）"），后续再用简称。
8. **质量 > 数量**：宁可全篇只有 5 条扎实新闻，也不要凑出 13 条注水内容。
